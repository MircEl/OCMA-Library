...
//changes background writing color with r g b
void OCMA::background(uint8_t r, uint8_t g, uint8_t b){
	bcolor=color(r, g, b);
...
//changes foregrownd writing color with r g b
void OCMA::stroke(uint8_t r, uint8_t g, uint8_t b){
	fcolor=color(r, g, b);
	stroke(color(r, g, b));
}
...
void OCMA::touchRead(void){
	if(tpmode!=1){										//custom touch panel mode
		if(tpmode!=5) touched=false;
		if(tpmode!=5) released=false;
		bool ok=true;
		int i=0;
		uint16_t x, y;
		while(read() != 0xfd){
